$(document).ready(function() {
  ...
  const WGrayIcon = L.Icon.extend({
    options: {
      iconUrl: 'westernGrayIcon.png', // Ensure this path is correct
  ...
  const EGrayIcon = L.Icon.extend({
    options: {
      iconUrl: 'easternGrayIcon-67-81.png',
  ...
  // Adding to 'map2'
  // L.marker([37.24, -122.0], { icon: eGrayIcon })
  //   .addTo(map2)
  ...
  // add squirrel markers 
 // iterate through squirrels and assign each an icon
  fetch('js/squirrels.json')
    .then(response => response.json())
    .then(sqData => {
      console.log("Squirrels data:", sqData); //is data loading?
      if (sqData && sqData.squirrels) {
        sqData.squirrels.forEach(squirrel => {
          console.log("Processing squirrel:", squirrel.title); // assign icons to squirrel type

          let icon;
          switch (squirrel.type) {
            case "Douglas Squirrel":
              icon = new DouglasIcon();
              break;
            case "Western Gray Squirrel":
              icon = new WGrayIcon();
              break;
            case "Humboldt's Flying Squirrel":
              icon = new FlyingIcon();
              break;
            case "Fox Squirrel":
              icon = new FoxIcon();
            ...
            case "Eastern Gray Squirrel":
              icon = new EGrayIcon();
              break;
            case "Ground Squirrel":
              icon = new GroundIcon();
              break;

            default:
              console.log("Unknown squirrel type:", squirrel.type);
              return;
          }

         
  // i want to add squirrel icon to screen only when one of the buttons is clicked
//and remove squirrel icon when the button is clicked again(toggle) - right now, the button triggers the layer on and off - want the button click also trigger the icons?
          
// i need to add event listeners to all 6 buttons

           // create marker with icon
          var marker = L.marker([squirrel.lat, squirrel.lng], { icon: icon }).addTo(map2);
          console.log("Marker added for:", squirrel.title); // check marker added
          marker.bindPopup(`<strong>${squirrel.title}</strong><br>${squirrel.description}`);
        });
      }
    })
